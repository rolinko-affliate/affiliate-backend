...
type AnalyticsService interface {
	...
	AffiliatesSearch(ctx context.Context, Country string, page, Offset int) ([]*domain.AnalyticsPublisherResponse, error)
...
func (s *analyticsService) AffiliatesSearch(ctx context.Context, Country string, page, Offset int) ([]*domain.AnalyticsPublisherResponse, error) {
	if page < 1 {
		page = 1
	}
	if Offset < 1 {
	...
	pages := (page - 1) * Offset
	publishers, err := s.analyticsRepo.AffiliatesSearch(ctx, Country, Offset, pages)
	if err != nil {
		return nil, err
	}

	// Transform each publisher to the full response format
	var responses []*domain.AnalyticsPublisherResponse
